...
export function SkillDrawer({
  ...
  const open = !!drawer;
  const title = (
    { view: '技能详情', edit: '编辑技能', move: '移动技能', add: '新建技能' } as const
  )[drawer?.mode ?? 'view'];
  ...
  const submitAdd = () => {
    ...
    if (kind === 'group') {
      if (state.skills.some((g) => g.name === v)) {
        toast('已存在同名大类');
        return;
      }
    ...
    // 叶子技能
    if (attrIds.length < 1 || attrIds.length > 3) {
      toast('请选择 1–3 项关联属性');
      return;
    }
    ...
    if (parent && parent.node.ch.some((c) => c.name === v)) {
      toast('该分类下已有同名技能');
      return;
    }
  ...
  return (
    <Drawer open={open} title={title} onClose={close} label="技能详情">
      <div ref={bodyRef}>
          ...
          <>
            ...
            <div className="fld">
              ...
              <input
                ...
                maxLength={12}
                placeholder={
                  kind === 'group' ? '例如：身体' : kind === 'sub' ? '例如：运动' : '例如：水彩'
                }
                value={name}
            ...
            {kind === 'sub' && (
              <div className="fld" key="pa-sub">
                ...
                {state.skills.map((g) => {
                  const on =
                    selectedParent === g.name || (!selectedParent && state.skills[0]?.id === g.id);
                  return (
                    <div
                      key={g.id}
                      className="choice"
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') setSelectedParent(g.name);
                      }}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
            ...
            {kind === 'leaf' && branches.length > 0 && (
              <>
                <div className="fld" key="pa-leaf">
                  ...
                  {branches.map((b) => {
                    const on =
                      selectedParent === b.path ||
                      (!selectedParent && branches[0]?.path === b.path);
                    return (
                      <div
                        key={b.path}
                        className="choice"
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') setSelectedParent(b.path);
                        }}
                        style={{
                          display: 'flex',
                          alignItems: 'center',
                  ...
                  {state.attrs.map((a) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          setAttrIds((prev) =>
                            on ? prev.filter((x) => x !== a.id) : [...prev, a.id].slice(0, 3),
                          )
                        }
                        onKeyDown={(e) => {
                          if (e.key === 'Enter')
                        ...
                        style={{
                          ...
                          borderRadius: 6,
                          background: on ? (a.tint ?? 'var(--panel)') : 'var(--card)',
        ...
        {drawer && drawer.mode !== 'add' && !leaf && (
          <p style={{ color: 'var(--text-3)' }}>技能不存在或已删除。</p>
        )}
          ...
          <>
            {drawer.confirm === 'archive' && (
              <div className="warn-box">
                ...
                <div className="form-ops">
                  ...
                  <button
                    className="btn-ghost"
                    onClick={() => setDrawer({ ...drawer, confirm: null })}
                  >
                    再想想
                  </button>
                </div>
            ...
            {!drawer.confirm && (
              <div className="d-ops">
                ...
                {!leaf.archived && (
                  <button onClick={() => setDrawer({ ...drawer, confirm: 'archive' })}>归档</button>
                )}
                <button
                  className="danger"
                  onClick={() => setDrawer({ ...drawer, confirm: 'delete' })}
                >
                  删除
                </button>
              </div>
            ...
            <div style={{ marginTop: 12, marginBottom: 8 }}>
              <span
                className="tier"
                style={{
                  color: tierColor(leaf.lv),
                  borderColor: tierColor(leaf.lv),
                  fontSize: 12,
                  padding: '3px 10px',
                }}
              >
              ...
              <div className="bar" style={{ maxWidth: 220 }}>
                <i
                  style={{
                    width: `${Math.round(leaf.frac * 100)}%`,
                    background: tierColor(leaf.lv),
                  }}
                />
              </div>
            </div>
            <div style={{ fontSize: 13, color: 'var(--text-2)' }}>
              累计打卡 <b style={{ color: 'var(--ink)' }}>{leaf.checkins}</b> 天 · 所属：
              {curPath || '根目录'}
            </div>
        ...
        {drawer && leaf && drawer.mode === 'edit' && (
          <>
            ...
            <div className="form-ops">
              ...
              <button className="btn-ghost" onClick={() => setDrawer({ ...drawer, mode: 'view' })}>
                取消
              </button>
            </div>
          </>
        ...
        {drawer && leaf && drawer.mode === 'move' && (
          <div className="fld">
            <label>
              把「{leaf.name}」移动到（当前：{curPath || '根目录'}）· 换父不丢历史
            </label>
            {branches.map((b) => {
              ...
              return (
                <div
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') submitMove(b.path);
                  }}
                  style={{
                    display: 'flex',
                    alignItems: 'center',
