...
export function AttrDrawer({
  ...
  const submit = () => {
    ...
    if (mode.kind === 'edit') {
      ...
      if (state.attrs.some((a) => a.name === n)) {
        toast('已有同名属性');
        return;
      }
      dispatch({ type: 'ATTR_ADD', name: n, en: e, palette });
      toast(
        state.attrs.length + 1 > 8
          ? '已创建 · 属性超过 8 个，雷达图已切换为排行视图'
          : `已创建属性「${n}」`,
      );
    }
    setMode({ kind: 'list' });
  ...
  const doDelete = (id: string) => {
    const a = state.attrs.find((x) => x.id === id);
    const refs = collectLeavesAll(state.skills).filter((l) =>
      l.attrs.some(([aid]) => aid === id),
    ).length;
            ...
            <React.Fragment key={a.id}>
              <div className="attr-row">
                ...
                <button
                  className="icon-btn"
                  disabled={i === 0}
                  onClick={() => dispatch({ type: 'ATTR_MOVE', index: i, dir: -1 })}
                ...
                <button
                  className="icon-btn"
                  disabled={i === state.attrs.length - 1}
                  onClick={() => dispatch({ type: 'ATTR_MOVE', index: i, dir: 1 })}
        ...
        <>
          ...
          {editing && (
            <p style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 8 }}>
              正在编辑：{editing.name} · 等级与经验不受改名改色影响
            </p>
          )}
        </>
      )}
