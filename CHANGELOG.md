...
- **开机自启动**：设置页新增开关，可让桌面版在登录系统后自动驻留托盘，
  开机不弹窗。开关**直接读写系统启动项**（Windows 的 `HKCU\...\Run`），
  不落数据库 —— 系统本身就是唯一真相，再存一份反而会出现「面板说开着、
  系统其实关了」的偏差。网页端（含手机通过局域网打开的页面）没有修改启动项的
  权限，开关呈现为不可用

### 修复

- **每日打卡提醒会因窗口隐藏而整个丢掉**：提醒原先依赖一个 60 秒间隔的定时器
  精确比对 `HH:MM`。窗口最小化到托盘、或长期切到后台时，WebView 会把定时器降频
  甚至整体挂起，目标那一分钟没被调度到，当天的提醒就直接丢了；固定间隔还会累积
  漂移，让 tick 慢慢滑过目标分钟。现在改为「已到点即判定」，并且每轮重算到下一个
  整分的距离（不再累积漂移），窗口重新可见或重新聚焦时再主动判定一次
- **打卡提醒的日期用 UTC 计算**：原先把去重日期写成 UTC 日期，北京时间早上 8 点
  之前会被算成前一天，可能同一天提醒两次或漏掉一次。改用本地时区
- **提醒与文案不符**：文案写着「今天还没有记录」，但此前无论是否已记录都会提醒。
  现已改为当天已记录就不再打扰
- **本地打过包后 `pnpm lint` 必然失败**：ESLint 的 `globalIgnores` 只挡住了
  `apps/web/out`，漏了 `src-tauri/resources/web`（打包时由 `copy-sidecars.mjs`
  拷进去的前端产物），于是 lint 去解析压缩后的 chunk。`.prettierignore` 一并补上，
  避免 `pnpm format` 反过来重写产物
- **`next dev` 把用户主目录当成工作区根**：主目录下有个无关的 `package-lock.json`，
  Next 据此向上推断 root 并监视整个用户目录。已在 `next.config.ts` 用
  `outputFileTracingRoot` 显式钉回仓库根
